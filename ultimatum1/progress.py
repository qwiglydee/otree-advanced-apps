...
def track_running_round(iteround: Round) -> bool:
...
def track_running_trial(trial: Trial) -> bool:
...
def advance_round(pagename: str, player: Player, iteround: Round) -> Round:
    ...
    if iteround.is_running and not track_running_round(iteround):
        iteround.complete()
...
def advance_trial(player: Player, iteround: Round, trial: Trial) -> Progress:
    ...
    if trial.is_running and not track_running_trial(trial):
        trial.complete()
        track_running_round(iteround)
